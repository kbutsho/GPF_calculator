...
export function validateYear(data) {
  ...
  if (!data.startYear || data.startYear < 1900 || data.startYear > 2200) {
    errors.startYear = "অর্থবছর ঠিক নেই";
  }
  if (data.openingBalance < 0) {
    errors.openingBalance = "ওপেনিং ব্যালেন্স ঋণাত্মক হতে পারে না";
  }
  if (!data.slabs.length) {
    errors.slabs = "অন্তত একটি স্ল্যাব দিন";
  }
  data.slabs.forEach((s, i) => {
    if (s.rate < 0 || s.rate > 100) errors[`slab_${i}`] = "রেট ০ থেকে ১০০-র মধ্যে হতে হবে";
  });
  if (data.rateMode === RateModes.CUSTOM && (data.depositRate < 0 || data.depositRate > 100)) {
    errors.depositRate = "রেট ০ থেকে ১০০-র মধ্যে হতে হবে";
  }
  return errors;
}
